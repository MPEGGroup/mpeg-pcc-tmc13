...
### `--octreeParallelMaxNodeSizeLog2=INT-VALUE`
Enables multiple geometry sub-streams (suitable for parallel coding).  A
parallel geometry sub-stream will be generated for each of the last
*VALUE* octree levels.  For example, a value of 8 generates sub-streams
for octree levels where the node size is less than or equal to 256.

No parallel sub-streams are generated when *VALUE* is 0 or 1.
